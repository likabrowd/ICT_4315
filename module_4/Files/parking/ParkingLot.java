...
/**
 ...
 Each lot holds a ParkingChargeStrategy that calculates charges using a pluggable algorithm (Strategy Pattern). The strategy is chosen at
 construction time via ParkingChargeStrategyFactory (Factory Pattern), but can be swapped at runtime via setStrategy().
 
 The separation of concerns here is intentional:
  - ParkingLot knows its physical properties (id, name, scan type, base rates).
  - ParkingChargeStrategyFactory decides which pricing algorithm to attach.
  - The chosen ParkingChargeStrategy performs the actual calculation.
 */
